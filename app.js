(function () {
  ...
  async function loadAccounts() {
    ...
    list.innerHTML = state.accounts.map((a) => (
      '<div class="list-group-item d-flex justify-content-between align-items-center">' +
        '<div>' +
          '<div class="mono">' + a.accountNumber + '</div>' +
          '<div class="small text-muted">' + a.type + ' · ' + a.currency + '</div>' +
          '<div class="small text-muted mono">Branch ' + a.branchCode + ' · Routing ' + a.routingCode + ' · ' + a.swiftBic + '</div>' +
        '</div>' +
  ...
  async function loadBankInfo() {
    ...
    try {
      const data = await api('/accounts/bank-info');
      const b = data.bankInfo;
      el.innerHTML =
        '<div>' + b.bankName + '</div>' +
        '<div>Branch code: ' + b.branchCode + '</div>' +
        '<div>Routing code: ' + b.routingCode + '</div>' +
        '<div>SWIFT/BIC: ' + b.swiftBic + '</div>';
    } catch (e) {
      el.textContent = '—';
    }
  }
    ...
    try {
      const data = await api('/transfers', {
        ...
        body: {
          ...
          toBranchCode: fd.get('toBranchCode') || undefined,
          toRoutingCode: fd.get('toRoutingCode') || undefined,
          toSwiftBic: fd.get('toSwiftBic') || undefined
        }
      });
  ...
  async function loadTransfers() {
    ...
    list.innerHTML = data.transfers.map((t) => (
      '<div class="border-bottom border-secondary-subtle py-2">' +
        ...
        (t.toSwiftBic ? '<div class="text-muted mono">Branch ' + (t.toBranchCode || '—') + ' · SWIFT/BIC ' + t.toSwiftBic + '</div>' : '') +
