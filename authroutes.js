...
// works immediately). Safe to call more than once: by default, any
// username that already exists is left untouched. Add ?reset=true to
// instead reset an existing seed user's password/PIN back to the known
// demo values (useful if an earlier broken seed left a bad hash) — this
// only ever touches the 5 usernames in SEED_USERS, never anyone else's
// account, and never touches balances/accounts/transactions for existing
// users, just password/PIN.
// Guard: visit /api/auth/seed-demo-data?adminKey=<ADMIN_KEY env var,
// default dev-only-admin-key> in a browser (add &reset=true to reset), or
// POST with an `x-admin-key: <same value>` header (body { reset: true }).
const SEED_USERS = [
...
async function seedDemoData(req, res) {
  const reset = req.query.reset === 'true' || (req.body && req.body.reset === true);
  try {
    const result = await store.withTransaction(async (draft) => {
      const created = [];
      const skipped = [];
      ...
      SEED_USERS.forEach((su, i) => {
        const existing = draft.users.find((u) => u.username === su.username || u.email === su.email);

        if (existing) {
          if (reset && existing.username === su.username) {
            existing.passwordHash = bcrypt.hashSync(SEED_PASSWORD, 10);
            existing.pinHash = bcrypt.hashSync(su.pin, 10);
            existing.pinFailedAttempts = 0;
            existing.pinLockedUntil = null;
            logEventInDraft(draft, { type: 'user.seed_reset', userId: existing.id, details: { username: su.username } });
            resetUsers.push(su.username);
          } else {
            skipped.push(su.username);
          }
          return;
        }
