...
| GET | `/accounts/bank-info` | user | this bank's branch code, routing code, SWIFT/BIC |
...
| GET/POST | `/transfers` | user | list / queue `{fromAccountId,toAccountNumber,amount,description,toBranchCode?,toRoutingCode?,toSwiftBic?}` |
...
- **Bank identifiers** — every account carries a `branchCode`, `routingCode`
  (an alias of `branchCode`, kept for clients that expect that name), and
  `swiftBic`, all fixed values for this single simulated bank (see
  `BANK_INFO` in `helpers.js`). `GET /api/accounts/bank-info` returns them
  directly for "receive money" style UI. `POST /api/transfers` accepts
  matching optional `toBranchCode` / `toRoutingCode` / `toSwiftBic` fields
  for transfers to another institution; since there's no real correspondent
  banking network here, such transfers are accepted and queued but settle as
  `failed` with an explanatory reason rather than actually moving funds
  externally. Transfers without those fields (or with a `toSwiftBic` that
  matches this bank's own) settle normally against a matching account
